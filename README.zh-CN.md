...
一个**自包含播放器**——不是改版、不是给已有游戏打补丁。它把谱面文件变成任何浏览器里都能玩的节奏游戏:

- **随处可玩** — 任意现代浏览器,电脑或手机,无需安装
- **谱面格式** — `.pez` / `.pec` / `.json`,单文件或整个文件夹;Phi 格式自动识别并在加载时转换
- **两种游玩视角** — 转盘谱面,以及全屏键盘 / 指针游玩
- **单文件构建** — `node build.js` 产出 `dist/rpe-player.html`,双击即用的独立文件
- **你的风格** — 内置默认资源包,也支持 `.zip` 资源包换音符皮肤、打击音效、歌词与动画背景
